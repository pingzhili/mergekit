models=("modelglue/which-4/hybrid_ffn_lv_mixed_moe" "modelglue/which-4/hybrid_block_lv_mixed_moe" "modelglue/which-4/hybrid_model_lv_mixed_moe" "modelglue/which-4/linear_merged_model")
i=0
for model in "${models[@]}"; do
    ((i++))
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --main_process_port 21001  -m lm_eval --model hf \
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --main_process_port 21001 ../bigcode-evaluation-harness/main.py \
      --model $model \
      --tasks humaneval \
