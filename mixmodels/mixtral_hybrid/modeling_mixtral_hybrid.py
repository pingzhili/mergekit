...
from .configuration_mixtral_hybrid import MixtralHybridConfig
...
_CONFIG_FOR_DOC = "MixtralHybridConfig"
...
class MixtralAttention(nn.Module):
    ...
    def __init__(self, config: MixtralHybridConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class MixtralBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralHybridConfig):
        super().__init__()
...
class MixtralHybridDecoderLayer(nn.Module):
...
class MixtralPreTrainedModel(PreTrainedModel):
    config_class = MixtralHybridConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MixtralHybridDecoderLayer"]
...
class MixtralHybridModel(MixtralPreTrainedModel):
    """
    ...
    def __init__(self, config: MixtralHybridConfig):
        ...
        self.layers = nn.ModuleList(
            [MixtralHybridDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MixtralHybridForCausalLM(MixtralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MixtralHybridModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, MixtralHybridForCausalLM

        >>> model = MixtralHybridForCausalLM.from_pretrained("mistralai/Mixtral-8x7B-v0.1")
